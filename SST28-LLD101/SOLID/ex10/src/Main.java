public class Main {

    public static void main(String[] args) {

        System.out.println("=== Transport Booking ===");

        TripRequest req = new TripRequest(
                "23BCS1010",
                new GeoPoint(12.97, 77.59),
                new GeoPoint(12.93, 77.62));

        DistanceService dist = new DefaultDistanceService();
        FarePolicy farePolicy = new DefaultFarePolicy();
        DriverService driver = new DefaultDriverService();
        PaymentService payment = new DefaultPaymentService();

        TransportBookingService svc = new TransportBookingService(
                dist, farePolicy, driver, payment);
        svc.book(req);
    }
}
